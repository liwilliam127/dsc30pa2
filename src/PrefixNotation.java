...
public class PrefixNotation {
    ...
    protected static class StrStack {
    ...
    public static int evaluate(String notation) {
        ...
        *Method that evaluates the prefix notation expressions to integer outputs
        ...
        String[] notation_str = notation.split(" ");
        for (int i = notation_str.length-1;i>-1;i-- ){
            if (notation_str[i].length()==1){
                if (notation_str[i].charAt(0)=='+'||notation_str[i].charAt(0)=='-'||
                        notation_str[i].charAt(0)=='*'||notation_str[i].charAt(0)=='/'){
                    String last_item= stack.pop();
                    String second_last_item= stack.pop();
                    int item = calculator(last_item, second_last_item,notation_str[i]);
                    stack.push(String.valueOf(item));
                }else{
                    stack.push(notation_str[i]);
                }
            }else{
                stack.push(notation_str[i]);
            }
    ...
    private static int calculator(String a,String b, String operator){
        /**
         *Helper function that recognizes string form operators and computes
         *@param String two string forms of numbers and one string
         * form operator
         * @return int form result of computing the expression
         * */
        int res=0;
